...
//
//	NDIS_STATUS     NTStatus= NDIS_STATUS_FAILURE;
//	ULONG           BufferLen=0;
//	UINT            Len=0;
//
...
//
//	BufferLen=64;
//	TotalMacBuffer=(PUCHAR)ExAllocatePool(NonPagedPool,BufferLen);
//	memset(TotalMacBuffer,0,BufferLen);
//
...
--*/
{
	...
	//
	{
		...
		if (Remaining)
		{
			...
			//DbgPrint("PRINTING PACKET INFORMATION  1 1 1");
	...
	)
{
	...
	dProtocolType = NDIS_GET_PACKET_PROTOCOL_TYPE(packet);
...
--*/
{
	...
	//
    if (RecvBufferPool != NULL)
    {
        NdisFreeBufferPool(RecvBufferPool);
        RecvBufferPool = NULL;
    }
	if (SendBufferPool != NULL)
    {
        NdisFreeBufferPool(SendBufferPool);
        SendBufferPool = NULL;
    }
