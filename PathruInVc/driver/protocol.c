...
--*/
{
	...
	//试着将初始化好的PADAPT变量赋到全局变量pAdaptTotal中，以在分发例程中用之
	pAdaptTotal=pAdapt;
}
...
--*/
{
	...
	//
	//加入自己的处理过程，处理经过自己标识的查询
	//
    PSECLAB_REQUEST            pReqContext;
	
	//判断是否为自己的查询
    //
    //  得到请求信息域.
    //
    pReqContext = CONTAINING_RECORD(NdisRequest, SECLAB_REQUEST, Request);
    
	if(pReqContext->Signal == REQUEST_SIGNAL)
	{
		//
        //  保存完成状态
        //
...
--*/
{
	...
	do
	{
	    Signal1=SECLAB_SIGNAL1_FROM_SEND_PKT(Packet);	
	    Signal2=SECLAB_SIGNAL2_FROM_SEND_PKT(Packet);	
	    Signal3=SECLAB_SIGNAL3_FROM_SEND_PKT(Packet);

	    if(Signal1 != SIGNAL1 || Signal2 != SIGNAL2 || Signal3 != SIGNAL3)
//		if(SECLAB_SEND_PKT_RSVD(Packet)->RefCount!=199 && SECLAB_SEND_PKT_RSVD(Packet)->RefCount!=200)
		{
			//DbgPrint("The Packet is not what we want");
		    break;
		}

		pIrp = SECLAB_IRP_FROM_SEND_PKT(Packet);

		if(pIrp->IoStatus.Status != STATUS_PENDING)
		{
			DbgPrint("look out the synchrony problem");
		}
        ...
        NdisInterlockedDecrement(&PendedSendCount);
...
--*/
{
	...
	pOriginalBuffer = SECLAB_RCV_PKT_TO_ORIGINAL_BUFFER(Packet);

    if (pOriginalBuffer != NULL)
    {
        //
        //  我们曾经把盛有所有数据的NDIS_BUFFER摘下并存放在包的reserved域
        //  现在我们把盛有部分数据的buffer摘下并连接上全部数据的buffer
        //
        NdisUnchainBufferAtFront(Packet, &pPartialBuffer);
        NdisChainBufferAtBack(Packet, pOriginalBuffer);
    ...
    if (Status == NDIS_STATUS_SUCCESS)
    {
        ...
        //  将此包排队
        //
        EnListPacket(pAdapt, Packet);
    }
    else
    {
        SecLabFreeReceivePacket(pAdapt, Packet);
    }
...
--*/
{
	...
	//限制收包数量
//	CurrentNum++;
//	if(CurrentNum>=DefineNum)
//		return Status;
    do
    {
        /*//因为是在广域网中应用所以用不着判断包头是否为自己的，
		////广域网中的包头格式不能个性化
		if (HeaderBufferSize != sizeof(NDISUIO_ETH_HEADER))
        {
            Status = NDIS_STATUS_NOT_ACCEPTED;
			//可以看出NdisUio不能拦截包，也就是说它在passthru的上层
            break;
        }
        ...
        */
        ...
        //  为排队此包分配资源
        //
        pRcvPacket = SecLabAllocateReceivePacket(
                        pAdapt,
        ...
        NdisMoveMappedMemory(pRcvData, HeaderBuffer, HeaderBufferSize);
      

        //
        //  看是否所有数据都在lookahead缓冲区中
        //
        if (PacketSize == LookAheadBufferSize)
        {
			...
			*/
			...
			NdisMoveMemory(pRcvData+HeaderBufferSize,
				           LookAheadBuffer,
						   LookAheadBufferSize);
            ...
            //  将此包放入接收包队列中
            //
            EnListPacket(pAdapt, pRcvPacket);
        }
        else
        {
            //
            //  分配一个NDIS_BUFFER结构来盛放接收到的数据，在当前的数据开始处
            //  偏移headbuffersize大小，这样NdisTransferData例程就可以在适当位置
            //  拷贝剩余数据
            //
            ...
            {
                //
                //  摘掉并保存原来的NDIS_BUFFER,其中存放着原来收到的全部数据
                //
                NdisUnchainBufferAtFront(pRcvPacket, &pOriginalNdisBuffer);
                SECLAB_RCV_PKT_TO_ORIGINAL_BUFFER(pRcvPacket) = pOriginalNdisBuffer;

                //
                //  将现在的存放着部分数据的buffer链接上，以供NdisTransferData操作
                //
...
--*/
{
	...
	do
	{
        ...
        {
            pCopyPacket = SecLabAllocateReceivePacket(
                            pAdapt,
                            TotalPacketLength,
                            &pCopyBuf
                            );
            ...
            if (pCopyPacket == NULL)
            {
                DbgPrint("ReceivePacket:  failed to alloc copy");
                break;
            }
